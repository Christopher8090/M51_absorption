pro temp_vert_plus_ph_neu, ii, a, comp, lu, ul, T_min, T_max, lambda_Q, QQ_grain, $
	TT_Q, Qp_grain_a, T_P, U_P, PROP, dT, dE, energy,$
	VOIT=VOIT,inclbigbang=inclbigbang,Tback=Tback
...
;            15.3.2007 Joerg Fischera, add key word 'Tback'
;            16.3.2007 Joerg Fischera, made modifications needed
; ***************************************************************************************
; COMMENT:
; THE PROGRAM DERIVES THE PROBABILITY DISTRIBUTION THAT A GRAIN
; OF CERTAIN SIZE 'A' HAS A CERTAIN TEMPERATURE 'T'. THE COOLING
; PROCESS IS ASSUMED TO BE CONTINUOUS AND ONLY THE HEATING PROCESS IS
; ASSUMED TO BE STOCHASTIC IN NATURE. FOR VERY SMALL GRAINS THERE WILL
; OCCURS AN UNPHYSICAL SITUATION AT LOW TEMPERATURES WHERE THE EMITTED
; PHOTONS WOULD BE LARGER THAN THE ENERGY CONTENT OF THE GRAINS. IT IS
; THEREFORE SUGGESTED TO CONSIDER THE HEATING BY THE CMB PHOTONS AS A
; CONTINUOUS PROCESS. THIS IS DONE BY NOT SETTING THE KEYWORD
; 'INCLBIGBANG' AND BY SETTING THE TEMPERATURE 'TEQUIL' TO 2.7. THIS
; IS ALSO NOT PHYSICAL BUT WILL AVOID A FEATURE IN THE EMISSION
; SPECTRUM WHICH COULD BE INTERPRETED AS WARM DUST EMISSION CAUSED BY
; HEATING THE GRAINS BY CMB PHOTONS. 
...
ENDIF
IF KEYWORD_SET(Tback) THEN BEGIN
    IF Tback GT 10 THEN BEGIN
        Qp_Tback=INTERPOL(ALOG(Qp_grain_a),ALOG(TT_Q),ALOG(Tback))
        Qp_Tback = exp(Qp_Tback)
    ENDIF ELSE BEGIN
        Qp_Tback = vergl1[0] * (Tback/10.)^2.0
    ENDELSE
ENDIF
...
;if KEYWORD_SET(CONT) then begin
    ...
    ;print,'a, energy absorbed [J/s]: ', a, energy
...
; ------------------ change begin 15.3.2007 Joerg Fischera --------------------
   IF KEYWORD_SET(Tback) THEN BEGIN
       gain = stb_e*Qp_Tback*Tback^4*4.D-1*!DPI*a^2
       TRANS2 = (TRANS2-gain)>0
   ENDIF
; ------------------ change end 15.3.2007 Joerg Fischera ----------------------
...
ENDIF ELSE BEGIN
  ...
  ; renormalisation
  IF Re1[0] GT R1 THEN Re1 = Re1*(R1/Re1[0])
  ...
  ; renormalisation
  IF Re2[0] GT R2 THEN Re2 = Re2*(R2/Re2[0])
...
;dfg
  ...
  ; startpoint
  l=0
  REPEAT l=l+1 UNTIL BB[l-1,l] GT 0   

  P[l-1] = 1.D-10

  FOR j = l,N-1 DO BEGIN

      ; ------ FIRST APPROXIMATION (Guhatakurta & Draine) -------

      P = p>1.d-50
      P[j] = - TOTAL(BB[j-1,0:j-1]*P[0:j-1],2) / BB[j-1,j] 
...
; the following lines are commented out 16.3.2007 Jg. Fischera
; they are replaced by the lines following afterwards ...
...
; end commenting out 16.3 2007 Jg Fischera

; the following lines are included 16.3.2007
        delta0 = MIN([Edep_min/4.,epsilon[j-1]/20.])
        ;to = to/delta0
        ;from = from/delta0

        N1 = ((to-from)/delta0)<1.D4
	NN = ROUND(N1)

	NN = MAX([NN,50])<400L
	delta = (to-from)/DOUBLE(NN-1)

	E = DINDGEN(NN)*delta + from
	Em = 0.5 * (E[0:NN-2] + E[1:NN-1])

; end of added lines 16.3. 2007, Jg Fischera
...
; lines modified 16.3.2007, Jg Fischera
; just include the if statement
        A2 = 0.
        B2 = 0.
        IF D2 NE 0 THEN BEGIN
...
;       ; THE APPROXIMATE DISTRIBUTION FUNCTION
;  following lines are commented out and replaced, 16.3.2007 Jg Fischera
...
;
;end commenting out, the approximation is replaced by:
         cc1 = -50.>(l1[j-1]*(Em-U_m[j-1]))<50.
        cc2 = -50.>(l2[j-1]*(Em-U_m[j-1]))<50.
        fm = (A2*EXP(cc1)-B2*EXP(cc2))+C2
; end modification 16.3.2007, Jg. Fischera
      ...
      ; include the following line: Jg Fischera 16.3.2007
      fm=fm>0.
