...
obsdir = root+'NUrad_M51a/saves/obs/'
figdir = root+'NUrad_M51a/figures/model/'
results_dir = root+'NUrad_M51a/results/'
;-----------------------------------------------------------------------------------------
...
;-------------------------------------------- READ DUST EMISSION DATA -----------------------------------------
model_label = model+'_q'+qyear+'_t'+stau+'_s'+strsfr+'_no'+strold+'_bd'+strbd+'_hd'+shd+'_zd'+szd+'_hd1_'+shd1+'_zd1_'+szd1+'_hs'+shs+'_zs'+szs+'_hs1_'+shs1+'_zs1_'+szs1+'_reff'+sreff+'_ell'+sellipt+'_f'+sffactor+'.xdr'

filename = emiss_indata+'diffuse_luminosity_'+model+'_q'+qyear+'_t'+stau+'_s'+strsfr+'_no'+strold+'_bd'+strbd+'_f'+sffactor+'.xdr'
restore, filename
dim_lambda = n_elements(lambda)
dust_flux_diff = dblarr(7, dim_lambda)	; diffuse dust flux for all components [Jy]
dust_flux_diff[0,*] = flux	; total
dust_flux_diff[1,*] = flux1	; main thick disk
dust_flux_diff[2,*] = flux2	; main thin disk
dust_flux_diff[3,*] = flux3	; inner thick disk
dust_flux_diff[4,*] = flux4	; inner thin disk
dust_flux_diff[5,*] = flux5	; outer thick disk
dust_flux_diff[6,*] = flux6	; outer thin disk
dust_flux_diff[0,*] = dust_flux_diff[1,*]+dust_flux_diff[2,*]+dust_flux_diff[3,*]+dust_flux_diff[4,*]+dust_flux_diff[5,*]+dust_flux_diff[6,*]

filename = emiss_indata+'localised_luminosity_'+model_label
restore, filename
dust_flux_local = dblarr(4, dim_lambda)	; local dust flux for all HII components [Jy]
dust_flux_local[0,*] = lum_template_scaled / flux_to_lum	; given in [W/Hz], convert to [Jy]
dust_flux_local[1,*] = lum_template_scaled4 / flux_to_lum
dust_flux_local[2,*] = lum_template_scaled6 / flux_to_lum
dust_flux_local[3,*] = lum_template_scaled7 / flux_to_lum
dust_flux_local_tot = dust_flux_local[0,*] + dust_flux_local[1,*] + dust_flux_local[2,*] + dust_flux_local[3,*]
;--------------------------------------------------------------------------------------------------------------
;---------------------------------------------- DUST EMISSION SED ---------------------------------------------
dust_flux_tot = dust_flux_local_tot + dust_flux_diff[0,*]	; total dust flux including local and diffuse
dust_flux_inner = dust_flux_diff[3,*] + dust_flux_diff[4,*]
dust_flux_main = dust_flux_diff[1,*] + dust_flux_diff[2,*] + dust_flux_local_tot	; all HII within main
dust_flux_outer = dust_flux_diff[5,*] + dust_flux_diff[6,*]

wavelength_dust = lambda * 1e-10	; dust wavelengths in [m]
nu = c/ wavelength_dust	; [Hz]
dnu = abs(nu[1:dim_lambda-1] - nu[0:dim_lambda-2])	; frequency element [Hz]
dust_lum_local_tot = dust_flux_local_tot * flux_to_lum	; convert from [Jy] to [W/Hz]
dust_lum_local_mean = 0.5 * (dust_lum_local_tot[1:(dim_lambda-1)] + dust_lum_local_tot[0:(dim_lambda-2)])
dust_integ_lum_local = total(dust_lum_local_mean * dnu)	; integrated local dust emission

dust_lum_diff = dust_flux_diff[0,*] * flux_to_lum  ; convert from [Jy] to [W/Hz]
dust_lum_diff_mean = 0.5 * (dust_lum_diff[1:(dim_lambda-1)] + dust_lum_diff[0:(dim_lambda-2)])
dust_integ_lum_diff = total(dust_lum_diff_mean * dnu) ; integrated total dust emission
print, 'dust luminosity: '+strtrim(dust_integ_lum_diff,1)+' [W]'

dust_lum_local_inner = dust_flux_inner * flux_to_lum  ; convert from [Jy] to [W/Hz]
dust_lum_local_mean = 0.5 * (dust_lum_local_inner[1:(dim_lambda-1)] + dust_lum_local_inner[0:(dim_lambda-2)])
dust_integ_lum_inner = total(dust_lum_local_mean * dnu)	;integrated inner disk dust emisison

dust_lum_local_main = (dust_flux_diff[1,*] + dust_flux_diff[2,*]) * flux_to_lum  ; convert from [Jy] to [W/Hz]
dust_lum_local_mean = 0.5 * (dust_lum_local_main[1:(dim_lambda-1)] + dust_lum_local_main[0:(dim_lambda-2)])
dust_integ_lum_main = total(dust_lum_local_mean * dnu)	; integrated main disk dust emisison

dust_lum_local_outer = dust_flux_outer * flux_to_lum  ; convert from [Jy] to [W/Hz]
dust_lum_local_mean = 0.5 * (dust_lum_local_outer[1:(dim_lambda-1)] + dust_lum_local_outer[0:(dim_lambda-2)])
dust_integ_lum_outer = total(dust_lum_local_mean * dnu)	; integrated outer disk dust emisison

F_factor = dust_integ_lum_local /(dust_integ_lum_local+int_integ_lum[0,1])
print, 'F_factor: '+strtrim(F_factor,1)
int_flux_local = int_flux_young *(F_factor * f_lambda)	; luminosity of the local component [W/Hz]
int_flux_young_all = int_flux_local + int_flux_young	; combine diffuse+local emission for young stars
int_flux_tot = int_flux_young_all + int_flux_old + int_flux_bulge ; combine total intrinsic stellar emission [Jy]
;------------------------------------------------------------------------------------------------------------
;---------------------------------------- COMBINE DUST+STELLAR SED ------------------------------------------
wavelength_dust = wavelength_dust * 1e6	; converts dust wavelengths from [m] to [um]
dust_overlap = wavelength_dust[where(wavelength_dust lt max(wavelength_nurad))]	; the wavelengths shared by stars+dust
uv_wavelength = wavelength_nurad[where(wavelength_nurad lt MIN(wavelength_dust))]	; stellar wavelengths excluding shared with dust
wave_all = [uv_wavelength, wavelength_dust]		; concatenated stellar+dust wavelengths (0.09-5000um)

app_flux_combined = INTERPOL(app_flux_tot, wavelength_nurad, wave_all, /SPLINE)	; interpolates apparent SED
int_flux_combined = INTERPOL(int_flux_tot, wavelength_nurad, wave_all, /SPLINE)	; interpolates intrinsic SED
dust_flux = INTERPOL(dust_flux_tot, wavelength_dust, wave_all, /SPLINE)	; interpolates dust SED

if (where(int_flux_combined lt 0))[0] ne -1 then begin
	app_flux_combined[where(app_flux_combined lt 0)] = 0.	; removes negative values from interpolating
endif
if (where(int_flux_combined lt 0))[0] ne -1 then begin
	int_flux_combined[where(int_flux_combined lt 0)] = 0.	; removes negative values from interpolating
endif
app_flux_combined[where(wave_all gt 5.8)] = 0.
int_flux_combined[where(wave_all gt 5.8)] = 0.
if (where(dust_flux lt 0))[0] ne -1 then begin
	dust_flux[where(dust_flux lt 0)] = 0.	; removes negative values from interpolating
endif
dust_flux[where(wave_all lt 1)] = 0.

tot_app_flux = app_flux_combined + dust_flux	; total emission (apparent stellar + dust)
tot_int_flux = int_flux_combined + dust_flux
;--------------------------------------------------------------------------------------------------------------
;--------------------------------------------------- ANALYSIS -------------------------------------------------
...
for i = 0, n_elements(wavelength)-1 do begin	; calculate residuals at available wavelengths
        wave_posi = where(min(abs(wave_all - wavelength[i])) eq abs(wave_all - wavelength[i]))
        resi[i] = ((int_jy[i] - tot_app_flux[wave_posi]) /int_jy[i]) * 100
...
peak_wave_global = wavelength_dust[where(max(dust_flux_tot) eq dust_flux_tot)]	; wavelength where emission peaks
peak_wave_inner = wavelength_dust[where(max(dust_flux_inner) eq dust_flux_inner)]
peak_wave_main = wavelength_dust[where(max(dust_flux_main) eq dust_flux_main)]
peak_wave_outer = wavelength_dust[where(max(dust_flux_outer) eq dust_flux_outer)]
print, 'peak global: '+strtrim(peak_wave_global,1)+', dust temp: '+strtrim(2898d/peak_wave_global,1)+' K'
print, 'peak inner: '+strtrim(peak_wave_inner,1)+', dust temp: '+strtrim(2898d/peak_wave_inner,1)+' K'
print, 'peak main: '+strtrim(peak_wave_main,1)+', dust temp: '+strtrim(2898d/peak_wave_main,1)+' K'
print, 'peak outer: '+strtrim(peak_wave_outer,1)+', dust temp: '+strtrim(2898d/peak_wave_outer,1)+' K'
print, ''
...
dust_emission_tot = dust_integ_lum_diff
dust_inner_emission = total(dust_integ_lum_inner) / dust_emission_tot
dust_main_emission = total(dust_integ_lum_main) / dust_emission_tot
dust_outer_emission = total(dust_integ_lum_outer) / dust_emission_tot
print, 'inner dust contribution: '+strtrim(dust_inner_emission*100,1)+' %'
print, 'main dust contribution: '+strtrim(dust_main_emission*100,1)+' %'
print, 'outer dust contribution: '+strtrim(dust_outer_emission*100,1)+' %'
print, ''
;--------------------------------------------------------------------------------------------------------------
app_flux_combined[where(wave_all gt max(wavelength_nurad))] = !VALUES.F_NaN      ; ignore interpolation beyond...
int_flux_combined[where(wave_all gt max(wavelength_nurad))] = !VALUES.F_NaN      ; wavelengths in nurad
;--------------------------------------------------------------------------------------------------------------
;------------------------------------------------ PLOT --------------------------------------------------------
xmin = 0.09
xmax = 1000
ymax = max(tot_app_flux) *1.8
ymin = ymax*2e-5
...
cgplot, wavelength_dust, dust_flux_local_tot, linestyle=5, color='grey', ytitle='Flux [Jy]', xrange=[xmin,xmax], yrange=[ymin,ymax],$
...
cgplot, wave_all, int_flux_combined, color='purple', linestyle=0, /overplot
...
cgplot, wave_all, tot_app_flux, color='black', linestyle=0, /overplot
cgscatter2d, wavelength, int_jy, fit=0, /overplot	; scatter plot of corresponding SED

cglegend, colors=['black','red','grey','blue','green','purple','black'], linestyle=[0,5,5,5,5,0,2], title=['Total','Diffuse dust','Localised dust','Young stellar','Old stellar','Intrinsic','Data'], $
...
;SAVE, tot_lum, dust_lum, wave_all, wavelength_nurad, wavelength_dust, int_lum_combined, app_lum_combined, $
...
save, morph_comp, wave_all, app_flux_combined, int_flux_combined, dust_flux, tot_app_flux, tot_int_flux, $
